...
새 스택 하네스를 만들어야 한다면 작성 가이드가 따로 있습니다 — 하네스 본체 저장소의 `.harness/stacks/authoring-guide.md`입니다. 스택을 **만드는 사람**용이라 설치본에는 배포하지 않지만, 가져오는 길은 한 줄입니다. 설치된 본체 버전은 이 프로젝트의 `.harness/harness-lock.json`(`baseHarness.version`)에 있고, 그 버전 태그로 본체 저장소를 얕게 받아 가이드를 엽니다.
