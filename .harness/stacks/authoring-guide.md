...
| # | 빗나가는 이유 | 실측 |
| --- | --- | --- |
| 1 | **버전이 의존성 줄에 없다** | Spring의 dependency-management를 쓰는 Boot 2 저장소가 `major 3` 규칙을 그대로 지나갔습니다. 버전은 `plugins { id("...") version "..." }`에 있었습니다. Maven은 `<parent>`·`<dependencyManagement>`·`<properties>`에 있습니다 |
...
| Java | 버전이 실제로 사는 자리에서 좌표를 더 냅니다(예: `plugin:org.springframework.boot`) |
