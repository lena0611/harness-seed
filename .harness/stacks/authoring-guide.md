...
**규칙을 한 저장소에서만 쓸 거라면 이 가이드는 필요 없습니다.** md 파일 하나로 끝납니다. 아래 표에서 자기 상황을 먼저 고르세요. 아래로 갈수록 무거워지고, 무거운 쪽이 주는 것은 "여러 저장소가 나눠 쓰고 중앙에서 버전을 올리는 것" 하나뿐입니다.

| 상황 | 하는 일 | 무게 |
| --- | --- | --- |
| 규칙을 이 저장소에서만 쓴다 | `.harness/project/domain-rules.md`(또는 `architecture-rules.md`·`workflow-rules.md`)에 규칙을 쓴다. 끝. | 문서 1개 |
| 규칙을 "스택"으로 붙이고, 어느 폴더 변경이 어느 규칙에 닿는지도 잡고 싶다 | 저장소 안에 폴더 하나를 만들어 `manifest.json`·`policies.json`·`instructions/*.md`를 두고 `stack:apply --preset-path <폴더>` | 파일 4개 |
| 여러 저장소가 같은 규칙을 나눠 쓰고, 중앙에서 버전을 올려 배포한다 | 이 가이드의 나머지 전부 | 저장소 1개 + 태그 운영 |

첫 줄이 왜 충분한가: `.harness/project/*` 문서는 **프로젝트 소유**라 하네스 업데이트가 덮지 않습니다. 에이전트는 CLAUDE.md의 읽기 목록을 통해 그 문서를 이미 읽습니다. 스택 하네스를 만들어도 최종 결과물은 결국 `.harness/project/stack-preset-rules.md`에 정착한 md입니다 — 목적지가 같습니다.

둘째 줄이 첫 줄과 다른 점: `policies.json`의 `ownedAreas`가 폴더 glob과 규칙을 잇기 때문에, 커밋 검사가 "이번 변경이 어느 규칙에 닿았다"를 말해 줍니다. 설치기도 `package.json`도 필요 없고 별도 저장소도 아닙니다.

셋째 줄을 고르는 기준은 저장소 개수 하나입니다. 저장소가 하나면 태그를 만들 이유가 없습니다.

**실례(2026-09-07).** PHP 백엔드 통합 저장소가 잘 쓰인 개발 규약 250줄을 갖고 스택 하네스를 만들려 했습니다. 규약을 절별로 갈라 보니 대부분이 그 저장소 고유의 API 이름과 구조에 묶여 있어 다른 PHP 저장소에는 올릴 수 없었습니다. **저장소가 하나라 첫 줄로 결정했고**, 두 번째 PHP 저장소가 생기는 날 겹치는 절만 뽑아 올리기로 했습니다(결정 109). 규칙의 품질이 아니라 저장소 개수가 판단을 정한 사례입니다.
...
2. **견본을 복사합니다.** 견본은 따로 없습니다 — **카탈로그에 이미 있는 스택 하네스가 곧 견본입니다.** 주소는 이 문서가 들고 있지 않습니다(저장소가 옮겨지면 낡습니다). 목록에서 확인해 clone 하세요.
   ```bash
...
**확인한 범위**: PHP(`composer.json`, 2026-09-07), Gradle·Maven, Python(`pyproject.toml`·`requirements.txt`), Go(`go.mod`) — 넷 다 견본을 복사해 실제로 만들고 픽스처까지 돌렸습니다(2026-09-11·09-14). 버전이 안 적히는 구성은 Gradle 의 dependency-management 와 Maven 의 `<parent>` 둘 다 실제로 통과시켜 본 뒤 막았습니다. 네 번 모두 "바꿀 곳 셋"(+④)은 사실이었고 판정 엔진 아래쪽은 손대지 않았습니다. 대신 **리더가 할 일이 런타임마다 달랐고**, 빗나가는 모양은 위 「규칙이 빗나가는 네 자리」에 모아 두었습니다. **아직 안 해 본 런타임(Ruby·Rust·Kotlin DSL 등)은 그 표의 네 질문을 자기 의존성 파일에 먼저 던져 보세요.**
...
### 규칙이 빗나가는 네 자리 — 전부 조용히 통과한다

견본의 판정은 **의존성 이름 → 버전 문자열** 지도를 만들고, manifest 규칙의 `package`를 그 지도에서 찾습니다. npm과 composer는 이름이 그대로 적히고 버전이 늘 같은 줄에 있어 이 모양이 맞습니다. **다른 런타임은 네 군데에서 빗나갑니다.**

실패 모양은 넷 다 같습니다 — 규칙이 안 걸리니 **맞지 않는 프로젝트가 차단되지 않고 그냥 설치됩니다.** 설치가 실패하는 것보다 나쁩니다. 실패는 보이지만 이건 안 보입니다.

| # | 빗나가는 이유 | 실측 |
| --- | --- | --- |
| 1 | **버전이 의존성 줄에 없다** | Spring의 dependency-management를 쓰는 Boot 2 저장소가 `major 3` 규칙을 그대로 지나갔습니다. 버전은 `plugins { id("...") version "..." }`에 있었습니다. **Maven도 같습니다** — Spring Boot 를 `<parent>` 로만 선언한 pom 역시 그대로 통과했습니다(적대적 리뷰에서 따로 확인). `<dependencyManagement>` 나 `<properties>` 변수로 두는 구성도 흔한데 그쪽은 재보지 않았습니다 |
| 2 | **이름이 안 맞는다** | Python은 대소문자와 `-`·`_`·`.`를 가리지 않습니다(PEP 503). PyPI 정식 표기가 `SQLAlchemy`·`typing_extensions`라, 규칙에 `sqlalchemy`라고 적으면 빗나갑니다 |
| 3 | **major가 이름 안에 있다** | Go는 major 2부터 모듈 경로에 붙습니다(`github.com/labstack/echo/v4`). 규칙이 `.../echo`면 v2 이상을 통째로 놓치는데, 성숙한 라이브러리는 대부분 v2 이상입니다 |
| 4 | **그 의존성이 이 프로젝트 것이 아니다** | Go의 `// indirect`는 남이 끌고 온 것입니다. 견본은 섹션을 구분하지 않으므로(npm은 전부 직접 선언) 그대로 두면 쓰지도 않는 프레임워크로 엉뚱하게 차단합니다 |

**넷 다 읽는 쪽에서 끝납니다.** 판정 엔진(`expected`·`incompatible` 대조, major 판정, 문구)은 한 줄도 건드리지 않습니다 — 네 런타임으로 확인했습니다.

| 런타임 | 리더가 추가로 해야 할 일 |
| --- | --- |
| PHP | 없음 — npm과 같은 모양입니다 |
| Java | 버전이 실제로 사는 자리에서 좌표를 더 냅니다 — Gradle 은 `plugins` 블록(`plugin:org.springframework.boot`), Maven 은 `<parent>`. 한 스택이 두 빌드 도구를 다 받는다면 한쪽만 내면 반쪽입니다 |
| Python | 이름을 정규화합니다(소문자 + `-_.` 를 `-` 하나로) |
| Go | 경로의 `/vN`에서 major를 만들고, `// indirect`는 건너뜁니다 |

2번에는 **manifest 작성 계약이 딸려 나옵니다.** 리더가 정규화해도 규칙 쪽이 정규화된 이름이어야 만납니다. 그런 런타임이면 자기 검사나 README에 "규칙의 `package`는 정규화된 이름으로 적는다"를 남기세요.

**버전을 어디서도 못 구하는 경우도 있습니다.** Python의 미고정 의존성(`dependencies = ["sqlalchemy"]`)은 선언 어디에도 버전이 없습니다 — Gradle처럼 "다른 자리"가 아예 없습니다. lock 파일을 읽든지, 그 런타임에서는 major 규칙을 쓰지 않든지 둘 중 하나입니다.

자기 런타임에서 위 넷 중 해당하는 구성을 **픽스처로 만들어 회귀에 넣으세요.** 통과해야 하는 것 하나와 차단돼야 하는 것 하나로는 부족합니다 — 빗나가는 쪽은 **차단돼야 하는데 통과하는** 픽스처라야 드러납니다.

**견본에 그 자리가 있습니다 — `compat-fixtures/`.** 폴더 하나가 대상 프로젝트 하나이고, 그 옆에 `expect.json` 으로 "차단이냐 통과냐, 어느 규칙이 걸려야 하냐"를 적습니다. 자기 검사가 **선언한 `incompatible` 규칙이 전부 어느 픽스처엔가 적혀 있는지**를 막고, 설치기 회귀가 **그 픽스처를 실제로 돌려 그 규칙이 정말 뜨는지**를 봅니다.

둘이 붙어야 의미가 있습니다 — 선언만 보면 리더가 못 보는 좌표를 적어 두고도 초록이 뜨고, 실행만 보면 규칙을 지워 버리면 통과합니다. **규칙만 적고 픽스처를 안 만들면 검사가 안 통과하므로**, 우리가 그 언어를 미리 알지 못해도 작성자가 자기 손으로 자기 함정을 찾게 됩니다.
...
### `baseHarness.ref` — 태그를 박을지 범위로 둘지

이 한 칸이 스택 운영 부담을 정합니다. **새로 설치할 때 어느 본체를 받는가**를 정하는 값이고, 이미 설치된 프로젝트에는 강제가 아닙니다(설치기가 "검증된 기준 ref이며 exact pin이 아니다"라고 직접 출력합니다).

**검증된 정확한 태그(`v0.2.142`)를 적습니다.** 범위 표기(`semver:^0.2.142`)는 본체가 지원하고 한 번 적고 끝이라 편하지만, **쓰지 마세요.**

이유는 릴리스 시점 검증이 미래를 보장하지 못하기 때문입니다. 스택 v1을 본체 v0.2.142 대상으로 검증해 릴리스한 뒤, 본체가 같은 `0.2.x` 안에서 호환되지 않는 v0.2.144를 내면, **스택은 아무것도 바꾸지 않았는데 신규 설치가 검증 안 된 조합을 받습니다.** 릴리스 때 "원격 최신으로 설치해 보기"를 돌려도 그 시점 이후에 나올 본체는 볼 수 없습니다(2026-09-07 외부 리뷰에서 이 구멍을 지적받아 범위 표기를 되돌렸습니다).

임의 상한(`<0.2.150`)도 문제를 미루는 것뿐입니다. **공통 하네스가 계약 파괴 시 버전 경계를 올리는 규율을 실제로 채택한 뒤에** 범위를 다시 검토합니다.

| | 값 |
| --- | --- |
| `ref` | 검증한 정확한 태그. `minVersion`과 같게 |
| `minVersion` | 그 태그의 버전. 설치를 멈추는 바닥 |

대신 릴리스마다 이 값을 올릴지 판단해야 합니다. **본체가 올랐다고 무조건 올리는 게 아닙니다** — 그 본체를 대상으로 검증했을 때만 올립니다. 검증하지 않은 본체를 가리키면 그게 바로 위 문제입니다.

`minVersion`은 어느 쪽을 고르든 필요합니다. "이 본체 미만이면 설치를 멈춘다"는 바닥이고, 스택이 본체의 새 기능을 쓰기 시작할 때만 올립니다.
