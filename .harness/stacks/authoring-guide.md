...
| 적는 방식 | 새 설치가 받는 것 | 스택 운영자가 할 일 |
| --- | --- | --- |
| `"ref": "v0.2.142"` | 정확히 그 버전 | 본체 릴리스마다 이 값을 올려야 한다. 안 올리면 새 프로젝트가 낡은 본체로 깔린다 |
| `"ref": "semver:^0.2.142"` | 그 범위의 최신 태그 | **한 번 적고 끝.** 최소 버전만 필요할 때 올린다 |

범위 표기는 본체가 이미 지원합니다. 설치 기록에는 실제로 깔린 구체 버전이 남으므로 무엇이 깔렸는지 추적됩니다.

**어느 쪽을 고르나.** 본체가 계약을 바꿀 때 메이저 번호를 올리는 규율이 있으면 범위가 낫습니다. 없으면 태그를 박는 편이 안전합니다 — 같은 마이너 안에서 계약이 바뀌면 검증 안 된 조합이 새 설치에 깔리고, 그 손해는 다음에 처음 설치하는 사람이 봅니다. 사내 공통 하네스는 아직 `0.2.x` 안에서 계약 변경을 해왔으므로, 확실하지 않으면 태그를 박고 릴리스마다 올리세요.
