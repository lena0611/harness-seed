...
- root는 공통 하네스 전용입니다.
- `.harness/bin/harness standards:list` / `stack:apply` / `stack:reset` / `stack:status` 로 스택 기준 적용을 제어합니다.
- `.harness/bin/harness templates:list` / `template:apply` / `template:reset` / `template:status` / `template:gap`으로 제품 템플릿 계약 적용과 갭 검사를 제어합니다.
...
- `.harness/bin/harness outdated`는 lock을 읽고 같은 SemVer caret 범위의 업데이트 후보만 조회합니다.
- `.harness/bin/harness update`는 lock을 읽고 현재 스택과 공통 하네스를 같은 SemVer 호환 범위 안에서 함께 갱신합니다.
...
- 필요 시 `.harness/bin/harness standards:list`로 외부 스택 하네스 후보를 조회한 뒤 일반 프로젝트에는 `npx ... init`, 관리자/고급 흐름에는 `.harness/bin/harness stack:apply --preset-git <repo-url> --ref <tag>` 실행
- 제품 템플릿이 필요할 때만 `.harness/bin/harness templates:list`로 후보를 조회한다. 기존 프로젝트는 `.harness/bin/harness template:apply --preset-git <repo-url> --contract-only`, 빈 프로젝트는 일반 적용 뒤 `.harness/bin/harness template:gap`으로 계약 상태를 확인한다.
