...
## 결정 로그 기록 기준 (write 게이트)

리트머스 한 줄: **"3주 뒤 누군가 '왜?'라고 묻겠는가 + 그 답이 코드·git·문서에 안 보이는가" — 둘 다 예일 때만 기록합니다** (결정 87, 멀티사이트 피드백 수용).

- 기록한다: 구조·계약 선택(포기한 대안 포함), 기준 계층 간 충돌 해결, 예외·waiver, 권고 뒤집기, 팀이 되물을 "왜".
- 기록하지 않는다: 일반 구현 세부사항, 커밋 메시지로 충분한 것, 한 번뿐이고 재론 가능성 없는 선택, git log·CHANGELOG가 이미 기록하는 것.
- 세션 재개가 읽는 것은 현행 파일뿐이므로(아카이브 미읽음, 안내 임계 400줄) 항목 수 자체가 컨텍스트 비용입니다 — 밀도가 가치입니다.
...
- 차단 강도: `확인 필수`(번복 승격·뒤집기 누락)는 기본 모드에서 경고와 필수 조치 계상까지입니다. 커밋 차단까지 원하는 프로젝트는 `.harness/policy/profile.json`의 `harnessMode`를 `strict`로 전환합니다.
