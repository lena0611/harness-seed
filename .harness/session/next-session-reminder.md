...
- 0.2.93 push/배포 완료: 양쪽 원격(origin/main + company/master = `73e197b`) + 태그 `v0.2.93` 양쪽, Policy Guard success(1m1s). CLI `ai-standard-cli` **0.2.4**(GitLab `9080ce5` + 태그, check + 21 tests). 정책 원문 `ai-standard/docs`는 `854b523`(뒤집기 기록 의무 `279a5d2` + 템플릿 문구 정리). 직전 배포: 0.2.92(`3615592`, CLI 0.2.3), 0.2.91(`d11aace`, CLI 0.2.2), 0.2.90(`17ff14a`, CLI 0.2.1). 스택 `baseHarness.minVersion` 추종 불필요(4연속 릴리스 모두 스택 의존 신규 계약 없음).
