...
- ▶ **[대기] 멀티사이트 CI 백스톱 — 러너만 남음**: 채널·웹훅(curl 실측 OK)·CI 변수(Masked/비Protected)·기획 저장소 allowlist·스케줄(30 7,12 * * 1-5, Seoul, dev) 전부 완료. yaml 수정본(블록 스칼라) push됨(commit 91579fda). **잡이 "no runners" stuck** — 프로젝트에 러너 없음 확인, 인프라팀에 docker 러너 요청 발송(회신 대기). 러너 붙으면 ▶ 1회 → 잡 로그에서 fetch 인증 통과 확인이 마지막 검증. 참고: 잡은 태그 없음 — 러너가 태그 필수면 yaml에 tags 추가 필요(멀티사이트 에이전트에게 지시).
