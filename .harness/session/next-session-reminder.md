...
- 현재 본체 버전: **0.2.94** — score-print 검수 후속(요약 분류 한 줄 압축 + strict 승격 안내). **score-print 에픽 종결**: 판정→0.2.90/91/92 구현→0.2.93 정책 동기화→현장 검수(P1~P6 전부 충족)→0.2.94 후속까지 완결. 상세는 body-roadmap 에픽 종결 절.
- 0.2.94 push/배포 완료: 양쪽 원격(origin/main + company/master = `e211784`) + 태그 `v0.2.94` 양쪽, Policy Guard success(52s). CLI `ai-standard-cli` **0.2.5**(GitLab `205edf5` + 태그, check + 21 tests). 정책 원문 `ai-standard/docs`는 `854b523`(뒤집기 기록 의무 `279a5d2` + 템플릿 문구 정리). 오늘 배포 이력: 0.2.90(`17ff14a`)→0.2.91(`d11aace`)→0.2.92(`3615592`)→0.2.93(`73e197b`)→0.2.94(`e211784`), CLI 0.2.1→0.2.5. 스택 `baseHarness.minVersion` 추종 불필요(전 릴리스 스택 의존 신규 계약 없음).
