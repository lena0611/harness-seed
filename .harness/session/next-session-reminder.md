...
- 현재 본체 버전: **0.2.101 배포 완료(2026-08-06, `92f499b` 양원격+태그, 동반 범프 스택 v0.2.8·템플릿 v0.2.7·CLI 0.2.11)** — 매핑 커버리지 강제: 매핑된 영역(+형제 폴더, 깊이 2 이상)에 새 파일이 들어왔는데 spec-map 기록도 판정도 없으면 커밋에서 안내·gate에서 push 차단. 판정 완료 표기 `(사양 없음)`/`(코드 없음)`을 1급 상태로 도입("아직 안 봤다"와 구분). 회귀 2종(총 118종). 근거·설계는 decision-log 결정 17~19. **배포 승인 대기.**
