...
- **실습 프로젝트**: `~/project/practice/spec-linkage-test` — vue3 스캐폴딩(템플릿 v0.2.8 기준으로 적용됨) + planning-example 연동. 문서연동 손으로 확인할 때 씀. 지우거나 실사용 프로젝트로 착각하지 말 것.
...
- ⏳ **[월요일 2026-08-10 순서 제약] 기획 저장소 서브그룹 이전을 연동 안내보다 먼저.** 기획 저장소 그룹이 `ai-standard/policies` 아래 `planning`/`development` 서브그룹으로 재편됐고, `multisite`·`planning-example`을 `policies/planning/`으로 옮겨야 한다. 사용자는 해당 그룹 **Maintainer라 Transfer 불가**(Transfer·Archive·Delete가 모두 Owner 권한 — 설정 화면에 블록 자체가 없음), Owner에게 월요일 요청 예정. **이전 → 그다음 멀티사이트팀 `/기획문서연동` 안내** 순서를 지킬 것: 연동이 먼저면 개발자마다 옛 URL이 `spec-lock.json`에 박혀 전원이 정합 검사에 걸리고 `--move-baseline`으로 기준을 재생성해야 한다(첫 정규 소비자의 첫 경험). 이전 후 후속: 리마인더 경로 표기 갱신 + 스킬/정본의 기획 저장소 안내 문구를 `policies/planning/<프로젝트>`로. CHANGELOG·decision-log의 옛 경로는 그 시점 사실 기록이라 유지.
