...
- ▶ **[진행 중 — CI 잡 구현 지시 전달됨(2026-08-18), 멀티사이트 담당자 완성 연락 대기]** 완성 연락 오면: ① 웹훅 등록(아래 URL 조립본, Push events+master 필터) ② 검증 7단계(웹훅 Test→이슈 생성→중복 방지→코멘트→자동 닫힘→수동 닫기 재오픈→그물) ③ issue-adapter.md 견본 복사 안내. 지시문에 포함된 핵심: workflow.rules에 trigger 추가 필수(기존 관문이 스케줄만 통과), 기존 spec-freshness 패턴 재사용, allow_failure.
