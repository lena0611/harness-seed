...
## 마지막 세션 마감 상태 (2026-08-07)
- 현재 본체 버전: **0.2.103 준비 완료(미커밋)** — 캐시 역할 분리(기준 본문/최신 git 객체/읽어볼 최신 사본 3자리), **정산=읽은 스냅샷만 기록**(미검토 커밋 오정산 차단), 경로 안전 전 구간(symlink·traversal), 미정산 출처를 manifest로 통일, 소스별 독립 처리, TTL 지문, 본문 기반 관련성. **자체 적대적 검토 2회 + 외부 보완 재리뷰 1회 반영.** 회귀 16종 추가(총 145종). 상세 decision-log 2026-08-07 결정 31~45. **0.2.102는 미배포 — 이 버전에 흡수.** 배포는 다음 주 월요일 오전 예정(사용자 지시).
