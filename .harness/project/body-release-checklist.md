...
- [ ] **릴리스 태그(`vX.Y.Z`)는 마지막 커밋에 찍는다.** 순서: 본체 변경 커밋 → `npm run release:version-net`(자기 버전 ↔ CHANGELOG 정합) → **태그** → 양쪽 원격에 브랜치·태그 push. (2026-09-07 이전에는 여기서 스택·템플릿 동반 범프를 기다려야 했다 — 태그를 먼저 찍으면 신규 설치가 한 세대 낡은 스택 ref를 받았기 때문이다. 카탈로그가 버전을 고정하지 않게 되면서 그 대기가 사라졌다.)
...
### 스택 하네스 · scaffold 템플릿
**본체 릴리스는 이들을 손대지 않습니다**(결정 108, 2026-09-07). 각 저장소가 자기 릴리스를 소유하고, 자기 `manifest.json`의 `baseHarness.ref`로 본체를 어떻게 따라갈지 스스로 정합니다(태그 고정 또는 `semver:` 범위 — 판단 표는 [authoring-guide.md](../stacks/authoring-guide.md)의 `baseHarness.ref` 절).

- [ ] **본체가 할 일은 없습니다.** 버전 정합 검사(`npm run release:version-net`)도 본체 자기 것(`package.json` ↔ `CHANGELOG` 최상단)만 봅니다. 위성 manifest를 찾아 써주지 않습니다 — 남이 운영하는 스택은 그 자리에 없기 때문입니다.
- [ ] 배포 카탈로그(`.harness/stacks/registry.json`·`.harness/templates/registry.json`)는 **버전을 고정하지 않습니다.** 설치 명령이 최신 태그로 해석되므로, 위성이 새 태그를 내면 본체 릴리스 없이 반영됩니다. 본체가 손댈 때는 **새 자산을 목록에 처음 올릴 때**뿐입니다.
...
- [ ] 위성 쪽 절차(자기 ref 선반영, 결번, 범위 표기 안전장치 회귀)는 그 저장소의 릴리스 문서가 소유합니다.
...
- [ ] **발송은 사용자가 한다.** Mattermost 수신 훅으로 보내며 채널에는 훅 이름으로 찍힌다. `--json`을 주면 `{text}` 형태로 나와 훅에 그대로 POST할 수 있다.
- [ ] **에이전트는 공지를 보내지 않는다.** 이 스크립트에는 네트워크 호출이 없다(stdout 전용) — 의도된 설계이고, 발송 경로를 스크립트에 넣지 않는다. 팀별 사전·사후 개별 안내도 보내지 않는다(결정 99).
