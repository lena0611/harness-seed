...
- [ ] **원격 코드 검증 CI는 없다**(0.2.151, 결정 114). 검증은 로컬 훅이 전부 한다 — pre-commit이 정책·문서·계약을, pre-push가 회귀 전량을 돈다. push 후 확인할 원격 결과는 없다.
- [ ] **⛔ 번복 배너 커밋을 먼저 단독으로 보내고, 문서·회귀 커밋을 나중에 보낸다**(2026-09-07·09-08 실측). 번복은 보통 반대로 말하던 문서를 고치는 작업과 한 몸이고 그 정합을 트리 전역 회귀로 잠그는데, 순서를 거꾸로 하면 **문서·회귀 커밋의 트리에 배너가 아직 없어 그 커밋이 자기 회귀에 걸린다**(`ceb429e` 실패). HEAD가 초록이어도 자기 검사를 통과하지 못하는 커밋이 이력에 남는다.
  - CI를 다시 켠다면 알아 둘 것: `harness:check:strict`를 `--base/--head`로 돌리는 순간, 그 diff 범위의 decision-log에 ⛔가 있으면 기준 동기화 후보가 **확인 필수로 승격**되어 반드시 실패한다(0.2.91 승격 규칙). 승격을 "확인했다"고 표시해 해소하는 수단은 없고 diff 범위에서 빠지기를 기다리는 것이 유일한 처리이므로, 위 순서가 그때는 사실상의 계약이 된다. 릴리스 태그를 찍는 push의 범위에도 ⛔를 넣지 않는다.
