...
### 스택 하네스
- [ ] **결번 대조(재사용 금지 번호)**: 스택 `v0.2.26` · 템플릿 `v0.2.25` — 0.2.118~120 유령 ref 사고의 잔여 결번(0.2.121 결정). 동반 범프 번호를 정할 때 이 목록을 건너뛴다. (스택 v0.2.25·템플릿 v0.2.24는 0.2.122가 결번을 놓치고 실물로 채웠으므로 그 실물이 정본 — 2026-08-14 정정 기록 참조.)
...
- [ ] 스택 ref 범프 시 한 세트: 본체 변경 커밋 → `npm run release:version-net -- --write`(스택·템플릿 manifest에 baseHarness 갱신) → 스택 저장소 버전 bump + **manifest 자기 ref(stackHarness.ref)를 새 태그 번호로 먼저 맞춤** + check + 커밋 + 태그 + push → 본체 registry.json에 새 스택 ref 반영 → `version-net -- --write` 재실행(템플릿 manifest에 requiredStackHarness 갱신) → 템플릿 저장소 버전 bump + **자기 ref(template.ref) 선반영** + 커밋 + 태그 + push → 본체 registry 템플릿 ref 반영 커밋 → **그 커밋에 본체 태그** → `version-net` all-ok 확인 → 양쪽 원격에 브랜치·태그 push. (태그를 앞에 찍으면 신규 설치가 낡은 스택 ref를 받는다 — 4단계 참고.)
- [ ] **자기 ref 선반영을 빼먹지 말 것**: version-net --write는 태그 번호를 모르므로 자기 ref를 한 세대 뒤로 남긴다. 역대 태그(스택 ~v0.2.30, 템플릿 ~v0.2.29)가 한 세대 뒤 self-ref를 담아온 결함을 2026-08-19에 발견 — 실피해 없고 소급 수정하지 않으며, 템플릿 v0.2.30부터 선반영이 정상이다.
