...
- [ ] **새 테스트를 만들면 그것을 실제로 돌리는 곳이 있는지 확인한다** — `npm run` 스크립트만 만들면 아무도 돌리지 않는다(2026-09-07 실측: 레지스트리 계약 테스트 둘이 그 상태로 방치돼 있었고, 외부 리뷰를 고친 회귀도 그 안에 있어 실행되지 않았다). 본체 관문은 `.harness/bin/guard.mjs`의 seed-mode 분기가 돌리는 것뿐이다.
