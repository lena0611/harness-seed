...
## 세션 기록은 작업 커밋에 담습니다 — 기록 전용 커밋은 만들지 않습니다

`.harness/session/*`(리마인더·결정 로그·입력 큐)만 바뀐 **기록 전용 커밋**은 만들지 않습니다. 코드가 0줄 바뀐 커밋이 히스토리에 섞이면 진짜 변경을 찾기 어렵고, 브랜치를 합칠 때 같은 기록 파일이 양쪽에서 바뀌어 충돌합니다(#48 clubadm 실측 — 작업 커밋 4개 뒤 기록 커밋 1개, 중간에도 두 번 섞임. 본체 자신도 최근 50커밋 중 14개가 그랬습니다).

기록 전용 커밋이 생기는 원천은 둘이고, 갈 곳이 다릅니다.

| 원천 | 어디로 |
| --- | --- |
| 작업하면서 알게 된 것 — 다음에 볼 것, 조심할 함정 | **알게 된 그 자리에서** 기록 파일에 적고, 그 코드 커밋에 **함께** 담습니다. 끝나고 적으면 늦습니다 — 미리 적어야 같은 커밋에 실립니다 |
| 작업이 끝난 뒤 일어난 일 — 배포됨, 회신 받음, 검증 결과 | 바꿀 코드가 없으니 커밋하지 않습니다. **이슈 트래커**(JIRA·GitLab 이슈)에 코멘트로 남깁니다 |

- 아래 pre-commit의 "기록 전용 커밋은 검사 생략" 경로는 그대로 둡니다. 그래도 생겼을 때 `--no-verify`로 우회하지 않게 하는 안전망이지, 그런 커밋을 권하는 것이 아닙니다.
- 기록 파일을 `.gitignore`로 빼는 방식(로컬 전용)은 채택하지 않았습니다(#48 제안 A 보류). 이미 추적 중인 파일은 ignore에 넣어도 빠지지 않고, 빼려면 `git rm --cached`가 필요한데 그것을 자동으로 하면 팀원이 pull하는 순간 그 사람 체크아웃에서 인계 내용이 사라집니다. 이 원칙으로 기록 전용 커밋이 실제로 줄어드는지 본 뒤 다시 판단합니다.
- 이 문서는 프로젝트 소유라 업데이트가 덮지 않습니다. 이미 설치된 프로젝트는 이 절을 자기 문서에 옮겨 적으면 됩니다.
...
- 스테이징 전부가 `.harness/session/` 아래인 기록 전용 커밋은 통합 검사를 자동 생략합니다(0.2.124). 다른 파일이 하나라도 섞이면 평소대로 전체 검사합니다. `--no-verify` 수동 우회 대신 이 자동 판정을 사용합니다. 위 「세션 기록은 작업 커밋에 담습니다」 절의 안전망이며, 그런 커밋을 만들라는 뜻은 아닙니다.
