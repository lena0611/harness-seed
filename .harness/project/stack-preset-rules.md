...
이 문서는 `.harness/bin/harness stack:apply`가 활성 스택 프리셋을 프로젝트 로컬 규칙으로 정착시키는 장소입니다.
...
- `harness:update` 기본 동작은 현재 적용된 스택 하네스와 공통 하네스를 함께 갱신하는 것입니다. 하나만 갱신할 때는 `.harness/bin/harness update --stack-only` 또는 `.harness/bin/harness update --base-only`를 사용합니다.
