...
| 용어 | 쉽게 말하면 | 실물 |
| --- | --- | --- |
| 기준 계층 | 회사 공통 → 스택 → 템플릿 → 프로젝트 → 개인 순으로 **쌓이는** 층 — 덮어쓰기가 아니라 공존 (쌓이는 순서와 이기는 순서는 다릅니다) | `standards-layers.md` |
| 충돌 해석 순서 | 층끼리 부딪히면 **누가 이기는지**의 순서표 — 회사 필수 차단 1순위, 사용자 명시 지시 2순위, 그다음 더 구체적인 층 | `standards-layers.md`의 "충돌 해석 순서" |
