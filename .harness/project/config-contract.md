...
## profile.json은 이 문서를 가리킨다 (0.2.142)

`profile.json`의 `notes`는 예전에 모든 키 설명을 그 안에 통째로 담고 있었습니다. 같은 내용이 두 곳에 있으면 한쪽이 낡습니다 — 실제로 그렇게 됐습니다. 이제 `notes`는 요약과 포인터만 두고, **키 계약의 정본은 이 문서**입니다.

## 기획 문서 연동의 집행 (0.2.142)

기획 문서 연동은 **커밋 검증에서 참고 정보만 알려주고 push를 막지 않습니다.** push 차단 모드(`specEnforcement: "gate"`)는 0.2.142에서 제거했습니다 — 실사용 프로젝트가 0이었고, 기획 저장소 접근 실패에 fail-open이라 "막힌다"는 보장 자체가 약했습니다. 남아 있는 `specEnforcement` 값은 아무도 읽지 않으므로 무해하며, 정리하고 싶으면 지우면 됩니다.
