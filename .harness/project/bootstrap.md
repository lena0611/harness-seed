...
**기획 문서가 연동된 프로젝트**(`.harness/spec-lock.json` 존재)는 캐시(`.harness/generated/spec-cache/`)의 기획 문서가 최우선 근거입니다. 기능 문서들의 "왜 필요한가/누가 쓰는가"로 문제·사용자를, "하지 않는 것"들로 비목표를, "확인 기준"들로 성공 기준 초안을 만들어 **출처 문서와 함께 제시하고 확인을 받습니다**. 확인 없이 기록하지 않으며(헌장은 개발팀의 합의 표면), 위험 영역·회귀 지점·프로젝트 상태처럼 기획 문서가 답할 수 없는 항목은 그대로 질문합니다.
