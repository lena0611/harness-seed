...
- 결정 요청 보드: `https://git.smartscore.kr/ai-standard/docs/-/boards` (라벨 `결정요청`, 컬럼: 미결정/논의중/닫힘=결정됨)
...
- 본체 작업 중 "이 기능이 미결정 결정 요청에 걸리는가"가 궁금하면 결정 요청 이슈를 조회합니다 — 이 저장소의 [issue-adapter.md](./issue-adapter.md)가 그 조회를 정의하며, push 완료 보고에도 열린 결정 요청 요약 한 줄이 붙습니다.

요지 (정본의 뼈대만):

- 하네스는 팀 단위 안전장치(M0, 완료)에서 회사 규범이 필요한 국면으로 진입했습니다. 실증 3건(이슈보드 무중력 전환·verify 소유권 후퇴·견본 기각)이 근거입니다.
- M1 결정 요청의 가시화 → M2 결정 주체·기록처 지정 → M3 파일럿(이슈보드 관점 1순위) → M4 실행화·전파 → M5 순환 운영.
