...
### 배경
2026-07-09 clubadm 실사용 리뷰가 접수되었습니다. clubadm은 회사 첫 실질 공통 하네스 소비자이므로, 이 문서는 단순 의견이 아니라 본체 채택성/안전성 개선 입력으로 승격합니다.
...
### 배경
2026-08-04 score-print 실사용 피드백(base 0.2.89 / vue3 스택 0.1.47)이로 접수되었습니다. 총평은 "기억·연속성 A급, 예방 C급" — 회귀 4건을 하네스가 못 막았고, 그중 2건은 하네스가 옳은 신호를 줬는데 노이즈에 묻혀 에이전트가 무시했습니다. 공통 뿌리 두 축: ①노이즈가 신호를 죽인다(P2·P3·P4), ②조언을 차단으로 승격해야 한다(P1·P6).
...
## 컨텍스트 다이어트 — "하지 마라 열거"에서 "검증 가능한 정의"로 (백로그)

- 배경: 앤트로픽이 Claude Code 시스템 프롬프트 80%+를 삭제하고도 코딩 평가 성능이 유지됐다(2026-07-24 공개, 해설: wikidocs.net/blog/@jaehong/26443). 핵심은 삭제량이 아니라 두 가지 — ① 나쁜 컨텍스트의 비용은 토큰이 아니라 **추론**(모순 지시 정리)이다 ② 남길 것은 행동 규칙이 아니라 제품 맥락·gotcha·팀 고유 지식·검증 가능한 산출물이다. 하네스 본체는 후자(팀 정보 + 실행 장치)라 삭제 대상이 아니라 기사가 "남겨라"고 한 층이지만, 우리 문서에도 기사가 지운 층(행동 지시·일반론)이 섞여 있다.
- 판단 잣대 두 문장 (2026-08-21 사용자 채택):
  1. **"모델의 능력이 올라가면서 사람이 해야 할 일이 '무엇을 하지 마라'를 열거하는 쪽에서 '무엇이 좋은 결과인지 검증 가능하게 정의하는' 쪽으로 이동했다."**
  2. **"컨텍스트 엔지니어링이 성숙한다는 것은 결국 소프트웨어 엔지니어링으로 되돌아온다는 뜻."** — 하네스의 기반(계약·테스트·형상관리)은 감가상각되지 않는 층이라는 근거이기도 하다.
- 최근 선례: 결정 86(행동 규칙 → verify 소유권 인터페이스), 결정 87(금지 열거 → 리트머스 판단 기준) — 이 방향의 첫 두 사례.
- 작업 후보 4건:
  1. **CLAUDE.md·진입점 다이어트**: "파일 시스템이 이미 말해주는 뻔한 정보 제거, gotcha 중심 재편" 기준으로 감사 1회.
  2. **문서 간 모순 지시 감사**: 의미 충돌은 자동 감지 불가(실측)이므로 정본 문서 전수 1회 — "기록하라"류 vs write 게이트 같은 겹침 탐색. 문서-구현 계약 전수 대조(위 백로그)와 같은 회차에 묶으면 효율적.
  3. **규칙→판단 축 전환 후보 선별**: 절대 규칙 중 "정답 지정"을 "정답 찾는 법 지정"으로 바꿀 수 있는 것. 단, 계약성 규칙(필수 차단, 커밋 양식, 결번)과 감사 요구 영역은 제외 — 기사 자신의 단서("정말 중요한 영역 제외")가 하네스 존재 이유와 일치한다.
  4. **감가상각 재평가 항목**: 모델 세대 교체 시 "이 지시, 아직 필요한가"를 묻는 단계를 업데이트/릴리스 절차에 추가. 멀티모델 환경(Sonnet 바닥) 주의 — 강한 모델 기준 다이어트는 약한 모델 경로를 조용히 깬다.
- 착수 조건: 평가 근거 없이 지우는 것은 검증이 아니라 기대다(기사 caveat ①) — 각 삭제·전환은 회귀 또는 실측 대조를 동반한다. 우선순위는 실사용 피드백 대응보다 낮음.
- **1회차 실행 (2026-08-21, 0.2.127, 결정 88)**: 후보 1 완료(CLAUDE.md 3중 서술 통합 -10%, session-start-alert 18→13항목 -21%, baseline 5,887→5,562 tok). 후보 2 부분 완료(결정 86 낡은 실행 주장 4곳 정정 + lint 모순 1건 — 전수는 후속). 후보 3 부분(결정 86·87이 첫 사례, 정책 원문 17문항 전환은 ai-standard/docs 동반 작업으로 이관). 후보 4 완료(체크리스트 "모델 세대 교체 릴리스" 절). 잔여: 후보 3 원문 동반 작업(ai-standard/docs 17문항 전환).
- **2회차 실행 (2026-08-21, 0.2.128)**: 후보 2 전수 감사 완결 — 실행 주장 축 37건 검토(신규 결함 0, 전부 회귀 근거 있는 정확한 서술), 최종화 규칙 재서술 8곳 판정(전체 재서술 2곳 — context-protocol·standards-layers — 포인터화, 표·어댑터·프로젝트 특화 6곳은 정당 판정), 기록 지시 축 충돌 0. 결론: 모순 밀도는 낮았고 핵심 결함은 1회차에서 이미 소진 — 이후는 새 문서 작성 시 결정 88 원칙(정본 한 곳+포인터)으로 예방.
