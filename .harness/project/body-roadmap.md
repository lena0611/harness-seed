...
- 배경: "문서가 구현보다 세게 말하는" 결함이 짧은 기간에 세 번 나왔다 — ① 훅 설치 안내가 존재하지 않는 명령(harness:hooks:install)을 가리킴(실물 E2E에서 발견), ② 컨텍스트 "두 단계 합계 8초"가 실제로는 최소 1초 재부여로 초과 가능(4차 리뷰 P2), ③ standards-layers의 "scan이 충돌 후보를 남긴다"가 실제로는 구조적 불일치만 감지(실측). 전부 사후 발견이었다.
- 안: 정본 문서(.harness/policy/**, .harness/project/**)의 실행 주장 문장 — "자동으로 ~합니다", "차단합니다", "감지합니다", "보장합니다", "~가 실행됩니다" — 을 전수 추출해 하나씩 실측 대조한다. 각 문장에 대해 (a) 구현과 일치 (b) 과장 → 문구 하향 (c) 미달 → 구현 보강 또는 하향 중 판정하고 결과를 decision-log에 남긴다.
- 방법 후보: 명령·스크립트명 언급은 package.json scripts와 기계 대조 가능(오타류 즉시 검출). 동작 주장은 회귀 픽스처로 재현 확인. 한 번에 다 하지 말고 문서 단위로 나눠 진행.
- 착수 조건: 멀티사이트 온보딩이 안정된 뒤(지금은 실사용 피드백 우선). 단, 새 문서를 쓸 때는 지금부터 "계약을 구현보다 세게 쓰지 않는다"(decision-log 결정 45)를 적용한다.
