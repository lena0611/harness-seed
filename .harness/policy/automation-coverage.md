...
| `spec-lock-screen-consistency` | 기준에 기록된 문서가 링크한 화면이 기준에 함께 있고 같은 시점인가. 매핑되지 않은 문서는 아무도 보지 않는다 | 자동 검사 (`harness:spec:status` — 어긋나면 실패) |
| `spec-mapping-coverage` | 커밋 안내: 매핑된 영역(과 형제 폴더)의 추가·수정 **구현 파일**만 본다(문서·설정·하네스 경로 제외 — 잡음 방지). 매핑 누락은 그 코드가 이후 기획 변경 안내에 걸리지 않는 사각지대를 만든다 | 자동 안내 (커밋 검증) |
...
## 확장 규칙
- 새로운 자동 검사를 추가하면 이 문서의 자동 검증 가능 표에 반영합니다. **표의 `검증` 칸에 적을 실행 경로가 없으면 아직 자동 검사가 아닙니다** — 검사 파일만 있고 아무도 부르지 않는 상태를 이 칸이 드러냅니다(2026-09-07 실측). 경로 확인 절차는 [sync-protocol.md](./sync-protocol.md)의 "변경 유형별 연결 확인 A".
