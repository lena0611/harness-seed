# 컨텍스트 합성 프로토콜

AI 에이전트에게 모든 문서를 한 번에 주입하지 않습니다. 항상 읽어야 하는 최소 기준, 작업에 따라 찾아 읽을 기준, 실제 변경을 수행하는 도구를 분리합니다.

## 목적

- 프롬프트가 길어져도 핵심 기준이 묻히지 않게 합니다.
- 작업과 무관한 기술스택, 템플릿, 프로젝트 문서를 과도하게 읽지 않게 합니다.
- 회사 공통 기준, 스택 기준, 템플릿 사용 계약, 프로젝트 로컬룰, 개인룰의 계층을 유지합니다.
- 자동 생성된 색인이나 요약을 진실 출처로 오해하지 않게 합니다.

## 기본 구분

| 구분 | 역할 | 예시 |
| --- | --- | --- |
| Always-on 기준 | 모든 작업에서 먼저 읽는 최소 운영 기준 | `CLAUDE.md`, `ai-standard-guiding-policy.md`, `session-start-alert.md`, `active-context.md` |
| 작업별 판단 컨텍스트 | 이번 요청과 관련된 기준, 충돌 우선순위, 영향 후보를 합성한 문서 | `.harness/session/task-context.md` |
| 생성 컨텍스트 | 프로젝트 구조를 훑어 만든 재생성 가능한 보조 자료 | `.harness/generated/project-map.md`, `.harness/generated/import-map.md` |
| 실행 도구 | 실제 검증, 적용, 업데이트를 수행하는 명령 | `harness:scan`, `harness:handoff`, `harness:sync`, `harness:check` |
| Visible trace | 개발자가 확인할 수 있는 판단 요약 | `[harness] request/context/impact/action/decision/verify` |

## 기준 계층

기준은 아래 순서로 쌓입니다. 이 순서는 설치와 이해의 기반 순서입니다.

1. 회사 공통 기준
2. 선택한 스택 하네스 기준
3. 적용된 템플릿 사용 계약
4. 프로젝트 로컬 기준
5. 개인 기준

생성 컨텍스트는 기준 계층이 아닙니다. 실제 코드와 문서를 빠르게 찾기 위한 보조 산출물입니다.

## Always-on 선정 기준

항상 읽는 최소 기준은 아래 조건을 모두 만족하는 문서만 둡니다.

- 모든 작업에서 위배 여부를 먼저 확인해야 합니다.
- 보안, 권한, 검증, 복구, 기록처럼 피해를 제한하는 기준을 담습니다.
- 현재 세션의 작업 상태와 미완료 맥락을 복구하는 데 필요합니다.
- 특정 도메인, 스택, 템플릿, 화면 구현에 종속되지 않습니다.
- 작업 본문을 묻을 정도로 길지 않아야 합니다.

스택 기준, 템플릿 계약, 프로젝트 도메인 규칙, 결정 로그 전체는 항상 읽는 기준이 아닙니다. 세션 재개, 충돌 판단, 작업 영향 범위에 따라 추가로 읽습니다.

## Visible Trace 원칙

에이전트의 원시 내부 추론을 그대로 저장하거나 출력하지 않습니다. 개발자가 확인해야 하는 것은 생각의 전문이 아니라 작업을 통제할 수 있는 판단 흔적입니다.

적용 범위는 실제 업무 진행 보고입니다. 구현, 버그 수정, 운영 업무, 조사, 기준 변경처럼 작업을 진행하고 상태를 공유하는 턴에서는 visible trace를 사용합니다. 단순 질문 응답, 잡담, by-the-way 확인, 메타 대화처럼 업무 진행 보고가 아닌 캐주얼 턴에는 이 형식을 강요하지 않습니다.

권장 trace 단위:

- `request`: 목표, 범위, **완료 조건**, 비목표
- `context`: 읽은 기준, 추가로 좁힌 문서, 생성 컨텍스트
- `impact`: 영향 파일군, 기준 충돌 후보, 검증 범위
- `action`: 실행 명령, 수정 범위, 생성 산출물
- `decision`: 선택한 기준, 예외, 보류 질문, 기록 위치
- `verify`: 무엇을 **어떤 범위로** 확인했는지 — `harness:check`·프로젝트 도구(husky·CI 등) 결과와 아직 실행하지 않은 검증

### 완료 조건은 관찰 가능한 결과로 적습니다

"코드 수정", "문서 갱신", "테스트 추가"는 **수행 항목**입니다. 그것을 완료 조건으로 적으면 파일이 존재하는 것만으로 완료가 되고, 실제 실행 경로에서 동작하는지는 아무도 묻지 않습니다.

완료 조건에는 실제 사용자와 실행 경로에서 확인할 결과를 적습니다. 작업 규모에 맞춰 **2~4개**로 제한합니다.

| 변경 유형 | 수행 항목(완료 조건 아님) | 완료 조건 |
| --- | --- | --- |
| 정책 변경 | "문서를 고쳤다" | 실제 동작과 **현행 안내 전부**가 같은 정책을 말한다 |
| 검사 추가 | "테스트를 추가했다" | 정상 실행 경로가 그 검사를 호출하고, 위반하면 의도한 실패가 전달된다 |
| 병합·복원 | "병합 코드를 넣었다" | 기존 프로젝트 값이 보존되고 필요한 값만 더해지며, 복원도 된다 |

### 검증은 확인한 범위대로 씁니다

- **정적 확인** = 코드·호출 경로를 읽어 확인한 것. **실행 확인** = 직접 돌려서 확인한 것. 둘을 같은 낱말로 쓰지 않습니다.
- 어떤 **커밋·환경**에서 확인했는지 적습니다. 이전 커밋의 성공은 현재 커밋의 성공이 아닙니다.
- 원격 검사(CI 등)는 **실행 중·미실행·실패**를 그대로 표시합니다. 결과를 보지 않았으면 "확인했다"가 아닙니다.
- 일부 검사만, 또는 일부 환경에서만 통과했으면 **"전량 통과"라고 쓰지 않습니다**(2026-09-07 실측: 로컬 통과만 보고 "전량 통과"라고 보고했고 리눅스 CI에서 실패했다).
- 실행 승인이 아직 없는 단계(최종화 규칙)에서는 정적 확인까지 진행하고, **남은 실행 검증을 명시**합니다.

### 완료 보고 전에 주장과 근거를 한 번 대조합니다

작성한 설명을 실제 변경 결과와 맞춰봅니다. 아래 주장을 했다면 각각 근거가 있는지 확인합니다.

| 주장 | 확인할 것 |
| --- | --- |
| "제거했다" | 실행되는 참조가 남아 있지 않은가 |
| "자동 검사한다" | 정상 경로가 실제로 호출하는가 |
| "보존한다" | 선행 단계에서 이미 덮어쓰지 않는가 |
| "정합을 맞췄다" | 다른 현행 안내에 반대 설명이 없는가 |
| "통과했다" | 어떤 커밋·환경·검사에서 확인했는가 |

근거가 없으면 표현을 낮추거나 필요한 확인을 마칩니다. **이미 해소된 문제를 다시 뒤지거나 무관한 영역까지 리뷰를 넓히지 않습니다** — 이 대조는 짧게 한 번입니다.

변경 유형별로 무엇을 함께 확인하는지는 [sync-protocol.md](./sync-protocol.md)의 "변경 유형별 연결 확인"이 정본입니다.

trace는 프롬프트 진행 중 짧은 상태 로그, `harness:handoff` 산출물, 에이전트 최종 응답에서 같은 언어로 반복될 수 있습니다. 응답 형식 자체는 런타임 행동이므로 정적 검사로 강제하지 않고, Claude/Codex/Copilot 어댑터의 주입 문구와 진입점 문서로 조건부 리마인드합니다. 장기 보존이 필요한 판단은 trace가 아니라 `decision-log.md`, `developer-input-queue.md`, `waivers.json`, 프로젝트 룰 문서 중 맞는 곳으로 승격합니다.

## 충돌 해석 순서

**정본은 [standards-layers.md](../project/standards-layers.md)의 "충돌 해석 순서"입니다.** 여기에 목록을 복제하지 않습니다.

이 문서가 더하는 것: **생성 컨텍스트는 기준이 아닙니다.** 원본 문서나 실제 코드와 충돌하면 항상 원본을 우선하고, 예외가 필요하면 `decision-log.md` 또는 `waivers.json`에 남깁니다.

## 진실 출처

| 항목 | 진실 출처 | 검증 |
| --- | --- | --- |
| 공통 운영 원칙 | `.harness/policy/*.md` | `.harness/bin/harness impact`, `.harness/bin/harness check` |
| 스택/템플릿 계약 | `stack-preset-rules.md`, `template-contract.md`, 외부 manifest/README | `.harness/bin/harness stack:status`, `.harness/bin/harness template:status`, `.harness/bin/harness check` |
| 프로젝트 로컬룰 | `.harness/project/*.md` | `.harness/bin/harness check`, 코드/설정 한쪽 변경 후보 중 실제 기준 변경 여부 확인 |
| 세션 상태 | `.harness/session/*.md` | 항상 읽는 최소 기준, 세션 재개 항목, 미해결 항목 확인 |
| 프로젝트 구조 요약 | 실제 코드와 설정 파일 | `.harness/bin/harness sync`로 재생성 |
| 작업별 판단 컨텍스트 | 실제 문서와 생성 컨텍스트의 조합 | 에이전트가 필요 시 `.harness/bin/harness context "<작업>"`로 재생성 |

`.harness/generated/**`와 `.harness/session/task-context.md`는 재생성 산출물입니다. 사람이 직접 편집해 기준으로 삼지 않습니다.

## 로컬룰 누적 대응

프로젝트 운영 기간이 길어지면 룰 문서가 늘어납니다. 하네스는 이 문제를 모든 문서를 매번 읽는 방식으로 풀지 않습니다.

- 항상 읽는 기준은 짧게 유지하고, 긴 프로젝트 룰은 에이전트 판단 컨텍스트로만 불러옵니다.
- 룰 문서는 인덱스, 적용 범위, 최신 요약, 상세 규칙을 분리합니다.
- 같은 판단이 반복되는 경우만 프로젝트 룰로 승격합니다.
- 더 이상 맞지 않는 룰은 바로 삭제하지 않고 변경 이유를 `decision-log.md`에 남긴 뒤 최신 기준으로 정리합니다.
- 오래된 사례, 긴 회고, 참고용 상세는 세션 로그나 세부 문서로 분리하고, 상위 룰 문서에는 현재 따라야 할 기준만 둡니다.
- `harness:context -- "<작업 설명>"`의 후보가 너무 많으면 `-- --limit <개수>`로 줄이고, 누락된 문서는 작업 중 직접 확인합니다. 개발자가 업무 지시 때마다 직접 실행할 필요는 없습니다.

로컬룰은 무제한 프롬프트 재료가 아니라 검색 가능한 기준 저장소입니다. 생성 컨텍스트는 이 저장소에서 이번 작업에 필요한 후보를 찾기 위한 보조 색인입니다.

## 작업 흐름

1. 새 세션은 `CLAUDE.md`의 항상 읽는 최소 기준을 먼저 따릅니다.
2. 큰 작업이나 생소한 영역을 다룰 때 `.harness/bin/harness sync`로 프로젝트 맵을 최신화합니다.
3. 에이전트가 `.harness/bin/harness context "<작업 설명>"`으로 이번 작업의 Agent Decision Context를 생성합니다.
4. 후보 문서를 실제로 읽고, 상충되는 기준이 있으면 충돌 해석 순서에 따라 판단합니다.
5. 코드나 기준을 바꾼 뒤 반복 패턴이 드러나면 `.harness/project/domain-rules.md`, `architecture-rules.md`, `coding-conventions.md`, `workflow-rules.md`, `commit-push-rules.md` 중 맞는 곳에 로컬룰 후보로 승격합니다.
6. 최종화(검증·커밋·푸시)는 `CLAUDE.md` 작업 원칙의 "최종화 규칙(정본)"을 따릅니다.

## 금지

- 생성 컨텍스트를 원본 문서보다 우선하지 않습니다.
- 외부 검색 결과나 개인 메모로 프로젝트 로컬룰을 자동 덮어쓰지 않습니다.
- 검증 없이 side effect가 있는 작업을 에이전트 추론만으로 실행하지 않습니다.
- 벡터 색인이나 요약 파일을 저장소의 단일 진실 출처로 취급하지 않습니다.

## 연결 프로젝트 (한 세션에서 두 저장소)

프론트와 백엔드처럼 저장소가 둘인데 한 개발자가 한 대화창에서 둘 다 다루는 경우를 위한 장치입니다(0.2.139, 해석 규칙 0.2.140).

- **선언(팀 공유, 커밋됨)**: `.harness/policy/profile.json`의 `linkedProjects`. 예: `{ "label": "백엔드", "repo": "https://git.smartscore.kr/…/common.git", "focus": "ss/multisite" }`. 저장소의 정체(git 주소)와 관심 폴더만 적고 **PC 경로는 적지 않습니다** — 사람마다 clone 위치가 달라서요. `path`는 선택 힌트(있고 실존하면 우선). 손으로 쓰지 않아도 됩니다 — 에이전트에게 "백엔드 저장소 연결해줘"(스킬 `/연결프로젝트`)라고 하면 `harness linked add --repo <폴더|주소> --focus … --label …`가 선언과 개인 접근 폴더를 함께 씁니다(0.2.141).
- **해석(PC별)**: 개발자가 어차피 열어 두는 파일 접근 권한 목록 `.claude/settings.local.json`의 `permissions.additionalDirectories`(공유 `settings.json`도 봄)에서 git remote가 `repo`와 일치하는 폴더를 찾습니다. 못 찾으면 세션 시작 블록이 "그 목록에 추가하세요"라고 안내합니다. 상태 확인: `.harness/bin/harness linked`. 해석기: `.harness/bin/linked-projects.mjs`.
- **동작**: 세션 시작 훅이 그 저장소의 기준 문서 위치(루트 CLAUDE.md, focus 폴더의 CLAUDE.md), 하네스 런처 경로, "커밋·푸시는 그 저장소 안에서", "그쪽 훅은 이 세션에서 안 돈다", 리마인더 최근 항목을 주입하고, 프롬프트 컨텍스트가 매 턴 한 줄로 상기합니다.
- **파일 접근의 정확한 의미**: Claude Code는 세션 폴더 밖 파일을 Read/Edit/Write 도구로 만질 때마다 권한을 묻습니다(불가능이 아니라 확인). `additionalDirectories`(또는 `--add-dir`)는 그 확인을 면제합니다. Bash는 이 경로 제한과 별개입니다.
- **왜 필요한가(2026-09-02 실측)**: Claude Code는 추가 디렉터리의 파일·스킬·명령은 열지만 그 저장소의 세션 훅·설정은 돌리지 않고, 중첩 CLAUDE.md(서비스 룰)는 파일을 읽어도 로드하지 않습니다. 루트 CLAUDE.md도 환경변수(`CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1`)를 켜야 실립니다. 같은 이름의 슬래시 명령은 하나로 합쳐져 이 저장소 것만 동작합니다.
- **무엇이 자동이고 무엇이 아닌가**: 연결 저장소의 **git 훅(커밋·푸시 검사)은 그 저장소에서 커밋하면 자동으로 돕니다** — 사람이 손으로 돌릴 일은 없습니다. 자동으로 안 되는 것은 그 저장소의 **세션 훅**(쓰기 시점 차단·컨텍스트 주입)인데, 본체 훅은 두 저장소가 동일해 주 폴더 것이 사실상 대신하고(`# scope: harness`), 빠지는 건 프로젝트·스택이 추가한 게이트(`# scope: project`, 예: PHP 호환 검사)뿐입니다. 하네스 업데이트·check도 한 세션에서 "양쪽 다 해줘"라고 말하면 에이전트가 그쪽 런처 경로로 실행합니다 — 창을 따로 열 필요 없습니다.
- **주 폴더 규칙(실측)**: 세션의 주 폴더는 반드시 하네스 저장소의 **루트**여야 합니다. 두 저장소의 공통 상위 폴더로 열면 어느 쪽 훅도 실행되지 않고, 모놀리스의 하위 서비스 폴더로 열어도 훅은 0개입니다(설정은 상위로 상속되지 않음). 두 경우 모두 CLAUDE.md는 도달하므로 소비자 CLAUDE.md의 "세션 주 폴더 확인" 항목이 에이전트에게 알리게 합니다.
- **옵트인 우회(개인 설정, 하네스는 생성하지 않음)**: `.claude/settings.local.json`의 `hooks`에 상대 저장소의 `scope: project` 훅을 얹을 수 있습니다 — 훅 등록은 설정 계층 간 병합되어 이 저장소 훅은 그대로 살고(실측), 얹은 훅은 자기 경로 규칙에만 발동합니다. 상대 훅이 `CLAUDE_PROJECT_DIR` 기준으로 루트를 잡으면 명령에 `CLAUDE_PROJECT_DIR=<상대 경로>`를 붙입니다(본체 훅은 절대경로 패턴이라 불필요). 함정: 경로 하드코딩, 상대 하네스 업데이트 시 조용히 어긋남 — 그래서 하네스가 대신 만들어 주지 않습니다(길라잡이 원칙).
- **프로젝트 전용 게이트의 권장 자리(2026-09-03)**: 연결 저장소의 세션 훅은 어느 창에서도 따라오지 않으므로, 팀 전용 규칙(예: PHP 7.2∩8.4 호환 문법)은 **룰 문서**(`domain-rules.md`·focus 폴더 CLAUDE.md — 연결 블록이 먼저 읽게 함)에 적고, **물리 검사는 그 저장소의 git pre-commit**(커밋은 항상 그 저장소에서 하므로 어느 창이든 통과)에 둡니다. 쓰기 시점 세션 훅은 그 저장소 창에서만 도는 덤입니다. 상대 훅을 세션에 얹어 주는 명령은 만들지 않습니다(위 옵트인 우회가 유일한 예외 경로).
- **경계**: 하네스는 연결 저장소를 읽기만 하고 수정하지 않습니다. 검사·기획 연동·설치 리포트는 각 저장소가 각자 수행합니다.

## 확장 방향

초기 단계에서는 로컬 파일 기반 색인과 키워드 매칭만 사용합니다. 중앙 검색 서버나 벡터 DB는 여러 저장소의 공통 정책 검색, 감사 로그, 접근 제어가 필요해질 때 별도 인프라로 확장합니다.
