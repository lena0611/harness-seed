...
## 반복 검증 완화
- `harness:check`는 같은 git tree가 이미 통과했으면 `.harness/generated/check-cache.json`을 사용해 **전체 검증(기준 동기화 후보 분석, doc-link, seed-mode의 test-init, 스택 raw verify와 verify 옵트인된 lint/test/build)을 통째로 스킵**합니다(0.2.70). 이들은 모두 git tree의 결정론적 함수이므로 "같은 tree면 결과가 같다"가 보장되어 검증 신뢰성을 해치지 않습니다. 캐시 키는 검사 모드(strict/default) + HEAD + 변경/핵심 파일 해시 + 스택 상태(`validationCacheKey`)이며, tree가 1비트라도 바뀌면 미스되어 전체 재검증합니다. 강제 재검증은 `--no-cache`.
