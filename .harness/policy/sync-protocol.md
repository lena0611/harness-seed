...
- `.harness/bin/scan-project.mjs`는 자동 감지 리포트까지만 생성하고, 프로젝트 기준 문서를 직접 덮어쓰지 않습니다. 기존 프로젝트의 AI 작업 룰 후보(`.cursor/rules/`, `.github/copilot-instructions.md`, `CLAUDE.md`, `AGENTS.md`, `docs/**/agent-rules.md` 등)는 `Existing AI Rule Document Candidates`에 보존/등록 판단 대상으로 표시하되 자동 삭제, 자동 병합, 자동 `profile.json sources[]` 등록은 하지 않습니다. 후보별로 `git tracked`, `.gitignore 적용됨`, `.gitignore 미적용` 상태를 표시해 개인/임시 기준의 커밋 위험을 드러냅니다. 팀 기준 등록은 `Existing AI Rule Registration Guide`에 `sources[]` 예시와 등록 효과를 표시합니다. 하네스가 설치한 `.claude/agents/`, `.claude/README.md`, `.github/copilot-instructions/` 보조 문서는 설치 전 기존 룰 후보로 보지 않습니다.
