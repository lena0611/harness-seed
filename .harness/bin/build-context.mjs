...
function renderContext() {
  ...
  // ① 팀 기준(lock) 문서 ② 기준 이후 바뀐 문서 ③ 아직 기준에 없는 신규 문서.
  ...
  if (relevantAdded.length > 0) {
    lines.push('**새로 올라온 기획 문서 (이번 작업과 관련)** — 아직 팀 기준에 편입되지 않았습니다.')
  ...
  if (unjudgedAdded.length > 0) {
    ...
    lines.push('**새로 올라온 기획 문서 (관련성 미판정)** — 파일명만으로는 이번 작업과의 관련을 판단할 수 없습니다. 제목을 확인하세요.')
